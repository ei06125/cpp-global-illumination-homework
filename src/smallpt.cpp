...
{ // Usage: time ./smallpt 5000 && xv image.ppm
    ...
    auto operator+(const Vec &b) const
    {
...
Vec radiance(const Ray &r, int depth, unsigned short *Xi)
{
    ...
    auto a = nt - nc, b = nt + nc, R0 = a * a / (b * b), c = 1 - (into ? -ddn : tdir.dot(n));
    auto Re = R0 + (1 - R0) * c * c * c * c * c, Tr = 1 - Re, P = .25 + .5 * Re, RP = Re / P, TP = Tr / (1 - P);
